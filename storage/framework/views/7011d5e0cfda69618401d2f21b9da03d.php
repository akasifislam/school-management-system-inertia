...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Noto+Serif+Bengali:wght@400;600;700&family=Noto+Sans+Bengali:wght@300;400;500;600;700&display=swap" rel="stylesheet">
...
<body>
  ...
  <?php if (!empty($announcements) && $announcements->isNotEmpty()): ?>
    <?php $__currentLoopData = $announcements->where('show_banner', true);
    $__env->addLoop($__currentLoopData);
    foreach ($__currentLoopData as $ann): $__env->incrementLoopIndices();
      $loop = $__env->getLastLoop(); ?>
      <?php
      $bc = ['info' => ['#dbeafe', '#1d4ed8', '#1e40af'], 'success' => ['#dcfce7', '#15803d', '#14532d'], 'warning' => ['#fef3c7', '#d97706', '#92400e'], 'danger' => ['#fee2e2', '#dc2626', '#991b1b']];
      $c = $bc[$ann->type] ?? $bc['info'];
      ?>
      <div style="background:<?php echo e($c[0]); ?>;border-bottom:2px solid <?php echo e($c[1]); ?>;padding:8px 18px;display:flex;align-items:center;gap:10px;font-size:13px;color:<?php echo e($c[2]); ?>;position:relative;z-index:10">
        ...
        <button onclick="this.parentElement.remove()" style="margin-left:auto;background:transparent;border:none;font-size:20px;cursor:pointer;color:<?php echo e($c[2]); ?>;opacity:.7;line-height:1" aria-label="Close">×</button>
      </div>
    <?php endforeach;
    $__env->popLoop();
    $loop = $__env->getLastLoop(); ?>
  ...
  <?php if (!empty($announcements) && $announcements->where('show_popup', true)->isNotEmpty()): ?>
    <?php $popup = $announcements->where('show_popup', true)->first();
    $bc2 = ['info' => ['#1A6DB5', '#dbeafe', '#1e40af'], 'success' => ['#15803d', '#dcfce7', '#14532d'], 'warning' => ['#d97706', '#fef3c7', '#92400e'], 'danger' => ['#dc2626', '#fee2e2', '#991b1b']];
    $pc = $bc2[$popup->type] ?? $bc2['info']; ?>
    <div id="annPopup" style="position:fixed;inset:0;background:rgba(0,0,0,.58);z-index:9999;display:flex;align-items:center;justify-content:center;padding:16px">
      <div style="background:#fff;border-radius:10px;max-width:460px;width:100%;box-shadow:0 25px 70px rgba(0,0,0,.35);overflow:hidden;animation:popIn .25s ease">
        <div style="background:<?php echo e($pc[0]); ?>;padding:15px 20px;display:flex;justify-content:space-between;align-items:center">
          <h3 style="color:#fff;font-size:15px;margin:0;font-family:var(--font)"><?php echo e($popup->title); ?></h3>
          <button onclick="document.getElementById('annPopup').remove()" style="background:transparent;border:none;color:rgba(255,255,255,.8);font-size:24px;cursor:pointer;line-height:1" aria-label="Close">×</button>
        </div>
        <div style="padding:18px 20px;font-size:13.5px;color:#263238;line-height:1.7;font-family:var(--font)"><?php echo e($popup->message); ?></div>
        <div style="padding:11px 20px;background:#f8f9fa;text-align:right;border-top:1px solid #eee">
          <button onclick="document.getElementById('annPopup').remove()" style="background:<?php echo e($pc[0]); ?>;color:#fff;border:none;padding:8px 24px;border-radius:4px;font-size:13px;cursor:pointer;font-family:var(--font);font-weight:600">ঠিক আছে</button>
        </div>
      </div>
    </div>
    <style>
  ...
  <div class="page-wrap" id="top">
    ...
    <header class="site-header" role="banner">
      <?php if (!empty($settings['banner'])): ?>
        ...
        <img
          src="<?php echo e(asset('storage/' . $settings['banner'])); ?>"
          class="hdr-banner"
          alt="<?php echo e($settings['school_name_bn'] ?? 'School Banner'); ?>"
          loading="eager"
          fetchpriority="high">
      ...
      <div class="nav-inner">
        ...
        <ul class="nav-list" id="navList" role="menubar">
          ...
          <li class="nav-li <?php echo e(request()->routeIs('students*') ? 'active' : ''); ?>" role="none">
            ...
            <ul class="nav-drop" role="menu">
              <li><a href="<?php echo e(route('students.count')); ?>" role="menuitem">অধ্যয়নরত শিক্ষার্থীর সংখ্যা</a></li>
              <li><a href="<?php echo e(route('students.list')); ?>" role="menuitem">অধ্যয়নরত শিক্ষার্থীর তালিকা</a></li>
            </ul>
          </li>
      ...
      <div class="content-grid">
        ...
        <aside class="sidebar" role="complementary" aria-label="সাইডবার">
          ...
          <div class="sw">
            ...
            <div class="principal-card">
              <?php if (!empty($principal->photo)): ?>
                <img
                  src="<?php echo e(asset('storage/' . $principal->photo)); ?>"
                  class="principal-photo"
                  alt="<?php echo e($principal->name ?? 'প্রধান শিক্ষক'); ?>"
                  loading="lazy">
              <?php else: ?>
          ...
          <div class="sw">
            ...
            <ul class="sw-links">
              <li><a href="https://eshikshabd.com/" target="_blank" rel="noopener noreferrer">ই-স্কুল</a></li>
              <li><a href="https://www.pathshala.gov.bd/" target="_blank" rel="noopener noreferrer">পাঠশালা</a></li>
              <li><a href="https://pdms.gov.bd/" target="_blank" rel="noopener noreferrer">পিডিএস (সরকারি মাধ্যমিক)</a></li>
              <li><a href="<?php echo e(route('admission.apply')); ?>" class="red-link">ভর্তি পরীক্ষার আবেদন</a></li>
            </ul>
          </div>
          ...
          <div class="sw">
            ...
            <ul class="sw-links">
              <li><a href="https://www.moedu.gov.bd/" target="_blank" rel="noopener noreferrer">শিক্ষা মন্ত্রণালয়</a></li>
              <li><a href="https://www.dshe.gov.bd/" target="_blank" rel="noopener noreferrer">মাধ্যমিক ও উচ্চ শিক্ষা অধিদপ্তর</a></li>
              <li><a href="https://www.banebeis.gov.bd/" target="_blank" rel="noopener noreferrer">ব্যানবেইজ</a></li>
              <li><a href="#" target="_blank">নায়েম</a></li>
              <li><a href="https://www.nctb.gov.bd/" target="_blank" rel="noopener noreferrer">এনসিটিবি</a></li>
    ...
    <footer class="site-footer" role="contentinfo">
      <div class="footer-inner">
        <div>
          ...
          <p>Email: <a href="mailto:devsvally@gmail.com" class="footer-link">devsvally@gmail.com</a></p>
        </div>
        <div class="footer-r">
...
<?php /**PATH C:\Users\mmdri\Downloads\goblav\resources\views/layouts/frontend.blade.php ENDPATH**/ ?>