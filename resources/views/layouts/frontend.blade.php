...
<head>
    ...
    <title>{{ $title ?? ($settings['school_name_bn'] ?? 'গভর্নমেন্ট ল্যাবরেটরি হাই স্কুল') }} - কুমিল্লা</title>
    ...
    <style>
        :root {
            --c-primary: {{ $settings['color_primary'] ?? '#1A6DB5' }};
            --c-primary-d: {{ $settings['color_primary_d'] ?? '#0D5A9E' }};
            --c-primary-l: {{ $settings['color_primary_l'] ?? '#2980C9' }};
            --c-accent: {{ $settings['color_accent'] ?? '#E53935' }};
            --c-cyan: {{ $settings['color_cyan'] ?? '#0097B2' }};
            --c-cyan-d: color-mix(in srgb, {{ $settings['color_cyan'] ?? '#0097B2' }} 80%, black);
            --c-green: {{ $settings['color_green'] ?? '#2E7D32' }};
        }
    </style>
...
<body>
    @if (!empty($announcements) && $announcements->isNotEmpty())
        @foreach ($announcements->where('show_banner', true) as $ann)
            @php
                $bc = [
            ...
            @endphp
            <div
                style="background:{{ $c[0] }};border-bottom:2px solid {{ $c[1] }};padding:8px 18px;display:flex;align-items:center;gap:10px;font-size:13px;color:{{ $c[2] }};position:relative;z-index:10">
                <i class="fas fa-bullhorn" style="color:{{ $c[1] }};flex-shrink:0"></i>
                <strong>{{ $ann->title }}:</strong>
                <span>{{ $ann->message }}</span>
                <button onclick="this.parentElement.remove()"
                    style="margin-left:auto;background:transparent;border:none;font-size:20px;cursor:pointer;color:{{ $c[2] }};opacity:.7;line-height:1"
                    aria-label="Close">×</button>
            </div>
        @endforeach
    @endif
    @if (!empty($announcements) && $announcements->where('show_popup', true)->isNotEmpty())
        @php
        ...
        <div id="annPopup"
            ...
            <div
                ...
                <div
                    style="background:{{ $pc[0] }};padding:15px 20px;display:flex;justify-content:space-between;align-items:center">
                    <h3 style="color:#fff;font-size:15px;margin:0;font-family:var(--font)">{{ $popup->title }}</h3>
                ...
                <div style="padding:11px 20px;background:#f8f9fa;text-align:right;border-top:1px solid #eee">
                    <button onclick="document.getElementById('annPopup').remove()"
                        style="background:{{ $pc[0] }};color:#fff;border:none;padding:8px 24px;border-radius:4px;font-size:13px;cursor:pointer;font-family:var(--font);font-weight:600">ঠিক
                        আছে</button>
                </div>
            </div>
        </div>
    ...
    <div class="page-wrap" id="top">
        <header class="site-header" role="banner">
            @if (!empty($settings['banner']))
                <img src="{{ asset('storage/' . $settings['banner']) }}" class="hdr-banner"
                    alt="{{ $settings['school_name_bn'] ?? 'School Banner' }}" loading="eager" fetchpriority="high">
                <div class="hdr-overlay">
                    @if (!empty($settings['logo']))
                        <img src="{{ asset('storage/' . $settings['logo']) }}" class="hdr-logo" alt="School Logo">
                    @else
                        <div class="hdr-logo-ph"><i class="fas fa-school"></i></div>
                    @endif
                    <div class="hdr-text">
                        <h1>{{ $settings['school_name_bn'] ?? 'গভর্নমেন্ট ল্যাবরেটরি হাই স্কুল' }}</h1>
        ...
        <nav class="main-nav" role="navigation" aria-label="প্রধান নেভিগেশন">
            <div class="nav-inner">
                ...
                <ul class="nav-list" id="navList" role="menubar">
                    ...
                    <li class="nav-li {{ request()->routeIs('home') ? 'active' : '' }}" role="none">
                        <a href="{{ route('home') }}" class="nav-link" role="menuitem">প্রথম পাতা</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('about*') ? 'active' : '' }}" role="none">
                        ...
                        <ul class="nav-drop" role="menu">
                            <li><a href="{{ route('about.overview') }}" role="menuitem">এক নজরে বিদ্যালয়</a></li>
                            <li><a href="{{ route('about.history') }}" role="menuitem">সংক্ষিপ্ত ইতিহাস</a></li>
                        </ul>
                    </li>

                    <li class="nav-li {{ request()->routeIs('information') ? 'active' : '' }}" role="none">
                        <a href="{{ route('information') }}" class="nav-link" role="menuitem">জনবল</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('students*') ? 'active' : '' }}" role="none">
                        <span class="nav-link" role="menuitem" tabindex="0" aria-haspopup="true">
                            শিক্ষার্থী <i class="fas fa-chevron-down chev"></i>
                        </span>
                        <ul class="nav-drop" role="menu">
                            <li><a href="{{ route('students.count') }}" role="menuitem">অধ্যয়নরত শিক্ষার্থীর
                                    সংখ্যা</a></li>
                            <li><a href="{{ route('students.list') }}" role="menuitem">অধ্যয়নরত শিক্ষার্থীর
                                    তালিকা</a></li>
                        </ul>
                    </li>

                    <li class="nav-li {{ request()->routeIs('results*') ? 'active' : '' }}" role="none">
                        <a href="{{ route('results') }}" class="nav-link" role="menuitem">পরীক্ষার ফলাফল</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('gallery') ? 'active' : '' }}" role="none">
                        <a href="{{ route('gallery') }}" class="nav-link" role="menuitem">ছবির গ্যালারী</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('contact') ? 'active' : '' }}" role="none">
                        <a href="{{ route('contact') }}" class="nav-link" role="menuitem">যোগাযোগ</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('apa') ? 'active' : '' }}" role="none">
                        <a href="{{ route('apa') }}" class="nav-link" role="menuitem">এপিএ</a>
                    </li>

                    <li class="nav-li {{ request()->routeIs('sudhachar') ? 'active' : '' }}" role="none">
                        <a href="{{ route('sudhachar') }}" class="nav-link" role="menuitem">সুধাচার কৌশল</a>
                    </li>
                ...
                <aside class="sidebar" role="complementary" aria-label="সাইডবার">
                    ...
                    <div class="sw">
                        ...
                        <div class="principal-card">
                            @if (!empty($principal->photo))
                                <img src="{{ asset('storage/' . $principal->photo) }}" class="principal-photo"
                                    alt="{{ $principal->name ?? 'প্রধান শিক্ষক' }}" loading="lazy">
                            @else
                                <div class="principal-ph"><i class="fas fa-user"></i></div>
                            @endif
                            <p class="principal-name">{{ $principal->name ?? 'রোকসানা ফেরদৌস মজুমদার' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="sw">
                        ...
                        <ul class="sw-links">
                            ...
                            <li><a href="{{ route('admission.apply') }}" class="red-link">ভর্তি পরীক্ষার আবেদন</a>
                            </li>
                        </ul>
                    </div>
                    <div class="sw">
                    ...
                    <div class="sw">
                        ...
                        <ul class="sw-links">
                            <li><a href="{{ route('admin.login') }}">ওয়েব মাস্টার লগইন</a></li>
                        </ul>
                    </div>
